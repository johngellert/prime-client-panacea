...
function MapPage(props) {
  ...
  const [lat, setLat] = useState(
    props.location.coordinates ? props.location.coordinates.lat : 0
  );
  const [lng, setLng] = useState(
    props.location.coordinates ? props.location.coordinates.lng : 0
  );
    ...
    async function fetchOrganizations() {
      try {
        const { data } = await axios.get(
          `/api/public/map?city_id=${props.location.city_id}&orgType=${props.location.orgType}`
        );
    ...
    return (
      <>
        <div className="container">
          ...
          <GoogleMapReact
            bootstrapURLKeys={{
              key: process.env.REACT_APP_GOOGLEMAPS_API_TOKEN,
              language: "en",
            }}
        ...
        {showOrganization.length > 0 ? (
          <>
            <div className="organization-list">
              <IconButton onClick={markerClose}>
